            ...
            <div class="col s3 backToPersonalInfo">
                <router-link :to="{name: 'personal-info'}">
                    <a class="waves-effect waves-light btn-small">
                        <i class="material-icons right">person</i>返回個人資料
                    </a>
                </router-link>
            </div>
        </div>
                    ...
                    <form class="col mainArea__form grey lighten-5 z-depth-2">
                        ...
                        <div class="row">
                            <div class="col s6 offset-s6">
                                ...
                                <input type="submit" class="btn teal lighten-3 ml-1"
                                       @click.prevent="edit_before_confirm">
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deleteUser(index) {
                this.$store.state.queryUserId = this.userData[index].id;
                console.log(this.userData[index].id);
                axios
                    .delete("/" + this.$store.state.queryUserId)
                    .then(res => {
                        console.log(res);
                        this.userListReload();
            ...
            read_selected_userData(index) {
                ...
                this.$store.state.queryUserId = userData.id;
                this.$store.state.userDataUpdate.id = userData.id;
            ...
            edit_before_confirm() {
                if (
                    this.$store.state.userDataUpdate.account !== "" &&
                    this.$store.state.userDataUpdate.password !== ""
                ) {
                    this.$store.commit("preloader_on");
                    axios
                        .get("?filter=" + this.$store.state.userDataUpdate.account)
                        .then(res => {
                            console.log(res.data);
                            if (res.data[0] && res.data[0].id !== this.$store.state.userDataUpdate.id) {
                                alert("此帳號已存在，請更換帳號");
                                this.$store.commit("preloader_off");
                            } else {
                                this.editConfirm();
                            }
                        })
            ...
            editConfirm() {
                ...
                // this.$store.state.userDataUpdate.address_valid
                    ...
                    axios
                        .put("/" + this.$store.state.queryUserId, {
